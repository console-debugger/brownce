...
// Edit prompt UI
const EditPrompt = ({ navigation, route }) => {

    const detail = route?.['params']

    const state = useSelector(state => { return state })
    const { ANSWER } = state['localeReducer']['locale']
    const { profileQuestions, myQuestionAnswerList } = state['profileReducer']
    ...
    useFocusEffect(
        useCallback(() => {
            const indexFound = myQuestionAnswerList.findIndex(item => { if (item['ProfileQuestionMasterId'] === detail['ProfileQuestionMasterId']) return item })
            ...
            for (let initialOne = 0; initialOne < profileQuestions.length; initialOne++) {
                for (let initialTwo = 0; initialTwo < myQuestionAnswerList.length; initialTwo++) {
                    if (profileQuestions[initialOne].ProfileQuestionMasterId === myQuestionAnswerList[initialTwo].ProfileQuestionMasterId) {
    ...
    const _navToAnswer = item => () => {
        navigation.navigate('editWriteAnswer', item['ProfileQuestionMasterId'] === detail['ProfileQuestionMasterId'] ? { ...item, selectedIndex: detail['index'], ...detail } : { ...item, selectedIndex: detail['index'], })
    }
